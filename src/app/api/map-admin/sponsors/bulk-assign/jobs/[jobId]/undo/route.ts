...
import { getSponsorBulkAssignJob, undoSponsorBulkAssignJob } from "@/lib/sponsor-bulk-assign-jobs";
...
export async function POST(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const reverted = await undoSponsorBulkAssignJob({
      jobId,
      channelId: job.channelId,
      actorUserId: sessionUser.id,
    });
    return NextResponse.json({ ok: true, job: reverted });
  } catch (error) {
    console.error("[api/map-admin/sponsors/bulk-assign/jobs/[jobId]/undo POST]", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "No se pudo deshacer el job." }, { status: 400 });
  }
}
