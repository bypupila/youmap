...
export async function GET(request: Request) {
  try {
    ...
    const payload = await loadReleaseReadiness();
    ...
    console.error("[api/admin/release/go-no-go GET]", error);
    return NextResponse.json({ error: "No se pudo cargar checklist go/no-go." }, { status: 500 });
  }
}
